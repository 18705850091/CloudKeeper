<template>
    <div class="uploader-panel">
        <div class="uploader-title">
            <span>上传任务</span>
            <span class="tips">（仅展示本次上传的任务）</span>
        </div>
        <div class="file-list">
                ...
                <div class="op">
                    ...
                    <div class="op-btn">
                        <span v-if="item.status === STATUS.uploading.value">
                            ...
                            <Icon v-else :width="28" class="btn-item" iconName="pause" title="暂停"
                                @click="pauseUpload(item.uid)">
                            </Icon>
                        </span>
                        <Icon :width="28" class="del btn-item" iconName="del" v-if="item.status != STATUS.init.value &&
                            item.status != STATUS.upload_seconds.value &&
                            item.status != STATUS.upload_finish.value" title="删除" @click="delUpload(item.uid,index)">
                        </Icon>
                         <Icon :width="28" class="clean btn-item" iconName="clean" v-if="
                             item.status == STATUS.upload_seconds.value ||
                             item.status == STATUS.upload_finish.value" title="清除" @click="delUpload(item.uid,index)">
                            </Icon>
                    </div>
                </div>
            </div>
